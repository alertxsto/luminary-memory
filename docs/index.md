...
- **Adaptive importance (v0.2.15)**, memories that keep getting recalled are re-estimated immediately, so frequently-used facts climb into the next turn's persistent context; pinned rules never downgrade.
...
- **Content-level anti-duplication (v0.2.15)**, core / persistent context / recall share one dedup set (ids + content hashes) so identical text appears exactly once per turn.
